...
typedef JoystickDirectionCallback = void Function(Offset offset);
...
class JoystickView extends StatelessWidget {
  ...
  DateTime? _processGesture(double size, double ignoreSize, Offset offset,
      DateTime? callbackTimestamp) {
    /*
    // Something like this seems like it should work better than what they have, but I haven't validated the math
    double middle = size / 2.0;
    final angle = offset.direction;
    final degrees = angle * 180 / _math.pi + 90;
    ...
    */
    ...
    // Since offset includes angle/degree math, I just return the offset..
    DateTime? newCallbackTimestamp = callbackTimestamp;
    if (onDirectionChanged != null &&
        _canCallOnDirectionChanged(callbackTimestamp)) {
      newCallbackTimestamp = DateTime.now();
      onDirectionChanged!(offset);
    }

    return newCallbackTimestamp;
  }
